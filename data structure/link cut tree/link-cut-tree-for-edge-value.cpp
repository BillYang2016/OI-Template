...
struct Link_Cut_Tree {
	...
	stack<int>S;
	Link_Cut_Tree() {
		for(int i=0; i<2*maxn; i++)tree[i].max=tree[i].val=-0x7fffffff/2;
	}
	...
	void link(int index,int x,int y,int val) { //index:id for edge
		link(x,index);
		link(index,y);
		tree[index].val=tree[index].max=tree[index].sum=val;
	}
	void modify(int index,int val) { //modify edge value 
		splay(index);
		tree[index].val=val;
		push_up(index);
	}
